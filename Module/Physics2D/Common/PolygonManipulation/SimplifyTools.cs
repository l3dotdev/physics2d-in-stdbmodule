...
public static class SimplifyTools
{
    ...
    public static Vertices CollinearSimplify(Vertices vertices, double collinearityTolerance = 0)
    {
        if (vertices.Count <= 3)
            return vertices;
    ...
    private static void SimplifySection(Vertices vertices, int i, int j, bool[] usePoint, double distanceTolerance)
    {
        if ((i + 1) == j)
            return;
    ...
    public static Vertices MergeParallelEdges(Vertices vertices, double tolerance)
    {
    ...
    {
        HashSet<Vector2> unique = [.. vertices];
        return [.. unique];
    }
    ...
    public static Vertices ReduceByArea(Vertices vertices, double areaTolerance)
    {
        ...
        for (int i = 0; i < vertices.Count; ++i, v2 = v3)
        {
            ...
            MathUtils.Cross(ref v1, ref v2, out double old1);
            MathUtils.Cross(ref v2, ref v3, out double old2);
            MathUtils.Cross(ref v1, ref v3, out double new1);
