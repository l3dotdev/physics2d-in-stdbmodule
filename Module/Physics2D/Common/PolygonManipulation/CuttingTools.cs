    ...
    public static void SplitShape(Fixture fixture, Vector2 entryPoint, Vector2 exitPoint, out Vertices first, out Vertices second)
    {
        ...
        foreach (Vector2 vertex in shape.Vertices)
        {
            if (vertex.Equals(localEntryPoint))
                localEntryPoint -= new Vector2(0, MathUtils.Epsilon);

            if (vertex.Equals(localExitPoint))
                localExitPoint += new Vector2(0, MathUtils.Epsilon);
        }
        ...
        for (int i = 0; i < vertices.Count; i++)
        {
            ...
            if (Vector2.Dot(MathUtils.Cross(localExitPoint - localEntryPoint, 1), vertices[i] - localEntryPoint) > MathUtils.Epsilon)
                n = 0;
            else
                n = 1;
        ...
        for (int n = 0; n < 2; n++)
        {
            ...
            newPolygon[n][cutAdded[n]] += MathUtils.Epsilon * offset;
            ...
            newPolygon[n][cutAdded[n] + 1] += MathUtils.Epsilon * offset;
        }
