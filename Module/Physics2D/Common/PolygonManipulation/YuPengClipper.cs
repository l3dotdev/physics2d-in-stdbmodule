...
public static class YuPengClipper
{
    ...
    private static List<Vertices> Execute(Vertices subject, Vertices clip, PolyClipType clipType, out PolyClipError error)
    {
        ...
        CalculateSimplicalChain(slicedSubject, out List<double> subjectCoeff, out List<Edge> subjectSimplices);
        CalculateSimplicalChain(slicedClip, out List<double> clipCoeff, out List<Edge> clipSimplices);
    ...
    private static double CalculateBeta(Vector2 point, Edge e, double coefficient)
    {
        double result = 0;
    ...
    private static double GetAlpha(Vector2 start, Vector2 end, Vector2 point)
    {
    ...
    private static double CalculateSimplexCoefficient(Vector2 a, Vector2 b, Vector2 c)
    {
    ...
    {
        Vertices polygon = [Vector2.Zero, edge.EdgeStart, edge.EdgeEnd];
    ...
    private static bool PointOnLineSegment(Vector2 start, Vector2 end, Vector2 point)
    {
        ...
        return MathUtils.Area(ref start, ref end, ref point) == 0 &&
                Vector2.Dot(point - start, segment) >= 0 &&
                Vector2.Dot(point - end, segment) <= 0;
    }
