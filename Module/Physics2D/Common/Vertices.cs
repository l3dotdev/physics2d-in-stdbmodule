...
public class Vertices : List<Vector2>
{
    ...
    public Vector2 GetCentroid()
    {
        ...
        for (int i = 0; i < Count; ++i)
        {
            ...
            double triangleArea = 0.5 * (current.X * next.Y - current.Y * next.X);
            area += triangleArea;
    ...
    {
        AABB aabb;
        Vector2 lowerBound = new Vector2(double.MaxValue, double.MaxValue);
        Vector2 upperBound = new Vector2(double.MinValue, double.MinValue);
    ...
    public PolygonError CheckPolygon()
    {
        ...
        for (int i = 0; i < Count; ++i)
        {
            int next = i + 1 < Count ? i + 1 : 0;
            Vector2 edge = this[next] - this[i];
            if (edge.LengthSquared <= MathUtils.Epsilon * MathUtils.Epsilon)
            {
    ...
    public void ProjectToAxis(ref Vector2 axis, out double min, out double max)
    {
        // To project a point on an axis use the dot product
        double dotProduct = Vector2.Dot(axis, this[0]);
        min = dotProduct;
        max = dotProduct;
    ...
    public int PointInPolygon(ref Vector2 point)
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            if (area == 0 && Vector2.Dot(point - p1, edge) >= 0 && Vector2.Dot(point - p2, edge) <= 0)
            {
                return 0;
            }
