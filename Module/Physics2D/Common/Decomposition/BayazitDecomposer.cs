...
internal static class BayazitDecomposer
{
    ...
    private static bool CanSee(int i, int j, Vertices vertices)
    {
        ...
        for (int k = 0; k < vertices.Count; ++k)
        {
            ...
            if (LineTools.LineIntersect(At(i, vertices), At(j, vertices), At(k, vertices), At(k + 1, vertices), out Vector2 _intersectionPoint))
                return false;
        }
        return true;
    }
    ...
    private static double SquareDist(Vector2 a, Vector2 b)
    {
        double dx = b.X - a.X;
        double dy = b.Y - a.Y;
        return dx * dx + dy * dy;
