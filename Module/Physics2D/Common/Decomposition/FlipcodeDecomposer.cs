...
internal static class FlipcodeDecomposer
{
    ...
    public static List<Vertices> ConvexPartition(Vertices vertices)
    {
        ...
        for (int v = nv - 1; nv > 2;)
        {
            ...
            if (Snip(vertices, u, v, w, nv, polygon))
            {
                ...
                // Output Triangle
                Vertices triangle = [_tmpA, _tmpB, _tmpC];
                result.Add(triangle);
    ...
    {
        //A cross bp
        double abp = (c.X - b.X) * (p.Y - b.Y) - (c.Y - b.Y) * (p.X - b.X);

        //A cross ap
        double aap = (b.X - a.X) * (p.Y - a.Y) - (b.Y - a.Y) * (p.X - a.X);

        //b cross cp
        double bcp = (a.X - c.X) * (p.Y - c.Y) - (a.Y - c.Y) * (p.X - c.X);
    ...
    {
        if (MathUtils.Epsilon > MathUtils.Area(ref _tmpA, ref _tmpB, ref _tmpC))
            return false;
