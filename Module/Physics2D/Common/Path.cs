...
public class Path
{
    ...
    public Vertices GetVertices(int divisions)
    {
        ...
        double timeStep = 1 / divisions;

        for (double i = 0; i < 1; i += timeStep)
        {
    ...
    public Vector2 GetPosition(double time)
    {
        ...
        if (Closed)
        {
            ...
            // relative time
            double lt = (time - _deltaT * p) / _deltaT;
    ...
    private void CalcCatmullRom(Vector2 p0, Vector2 p1, Vector2 p2, Vector2 p3, double amount, out Vector2 result)
    {
    ...
    public Vector2 GetPositionNormal(double time)
    {
        double offsetTime = time + 0.0001;
    ...
    public List<Vector3> SubdivideEvenly(int divisions)
    {
        ...
        double deltaLength = length / divisions + 0.001;
        double t = 0.000;
        ...
        while (deltaLength * 0.5 >= Vector2.Distance(start, end))
        {
            end = GetPosition(t);
            t += 0.0001;
