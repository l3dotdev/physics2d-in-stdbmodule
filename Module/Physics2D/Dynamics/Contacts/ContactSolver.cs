...
public class ContactSolver
{
        ...
        int[] locks, int velocityConstraintsMultithreadThreshold, int positionConstraintsMultithreadThreshold)
    {
        ...
        if (_velocityConstraints == null || _velocityConstraints.Length < count)
        {
            ...
            newBufferCount = newBufferCount + (newBufferCount * 2 >> 4); // grow by x1.125
    ...
    public void InitializeVelocityConstraints()
    {
        for (int i = 0; i < _count; ++i)
        {
            ...
            for (int j = 0; j < pointCount; ++j)
            {
                ...
                double rnA = MathUtils.Cross(ref vcp.rA, ref vc.normal);
                double rnB = MathUtils.Cross(ref vcp.rB, ref vc.normal);
                ...
                double rtA = MathUtils.Cross(ref vcp.rA, ref tangent);
                double rtB = MathUtils.Cross(ref vcp.rB, ref tangent);
                ...
                vcp.velocityBias = 0.0;
                double vRel = Vector2.Dot(vc.normal, vB + MathUtils.Cross(wB, ref vcp.rB) - vA - MathUtils.Cross(wA, ref vcp.rA));
            ...
            if (vc.pointCount == 2)
            {
                ...
                double rn1A = MathUtils.Cross(ref vcp1.rA, ref vc.normal);
                double rn1B = MathUtils.Cross(ref vcp1.rB, ref vc.normal);
                double rn2A = MathUtils.Cross(ref vcp2.rA, ref vc.normal);
                double rn2B = MathUtils.Cross(ref vcp2.rB, ref vc.normal);
    ...
    public void SolveVelocityConstraints()
    {
        ...
        {
            if (_count == 0) return;
            var batchSize = (int)Math.Ceiling((double)_count / Environment.ProcessorCount);
            var batches = (int)Math.Ceiling((double)_count / batchSize);
    ...
    private void SolveVelocityConstraints(int start, int end)
    {
        for (int i = start; i < end; ++i)
        {
            ...
            for (int j = 0; j < pointCount; ++j)
            {
                ...
                double vt = Vector2.Dot(dv, tangent) - vc.tangentSpeed;
                double lambda = vcp.tangentMass * (-vt);

                // b2Clamp the accumulated force
                double maxFriction = friction * vcp.normalImpulse;
                double newImpulse = MathUtils.Clamp(vcp.tangentImpulse + lambda, -maxFriction, maxFriction);
            ...
            if (vc.pointCount == 1)
            {
                ...
                double vn = Vector2.Dot(dv, normal);
                double lambda = -vcp.normalMass * (vn - vcp.velocityBias);

                // b2Clamp the accumulated impulse
                double newImpulse = Math.Max(vcp.normalImpulse + lambda, 0.0);
            ...
            else
            {
                ...
                double vn1 = Vector2.Dot(dv1, normal);
                double vn2 = Vector2.Dot(dv2, normal);
...
#pragma warning restore CS0219 // Variable is assigned but its value is never used
    ...
    private bool SolvePositionConstraints(int start, int end)
    {
        ...
        for (int i = start; i < end; ++i)
        {
            ...
            for (int j = 0; j < pointCount; ++j)
            {
                ...
                PositionSolverManifold.Initialize(pc, ref xfA, ref xfB, j, out Vector2 normal, out Vector2 point, out double separation);
                ...
                double C = MathUtils.Clamp(Settings.Baumgarte * (separation + Settings.LinearSlop), -Settings.MaxLinearCorrection, 0.0);

                // Compute the effective mass.
                double rnA = MathUtils.Cross(ref rA, ref normal);
                double rnB = MathUtils.Cross(ref rB, ref normal);
    ...
    public static class WorldManifold
    {
        ...
        public static void Initialize(ref Manifold manifold, ref Transform xfA, double radiusA, ref Transform xfB, double radiusB, out Vector2 normal, out FixedArray2<Vector2> points)
        {
            normal = Vector2.Zero;
                ...
                case ManifoldType.Circles:
                    {
                        ...
                        if (Vector2.DistanceSquared(pointA, pointB) > MathUtils.Epsilon * MathUtils.Epsilon)
                        {
    ...
    {
        public static void Initialize(ContactPositionConstraint pc, ref Transform xfA, ref Transform xfB, int index, out Vector2 normal, out Vector2 point, out double separation)
        {
