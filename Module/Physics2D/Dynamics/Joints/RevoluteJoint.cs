...
public class RevoluteJoint : Joint
{
    ...
    private double _motorMass;	    // effective mass for motor/limit angular constraint.
    ...
    internal override void InitVelocityConstraints(ref SolverData data)
    {
        ...
        {
            double jointAngle = aB - aA - ReferenceAngle;
            if (Math.Abs(_upperAngle - _lowerAngle) < 2.0 * Settings.AngularSlop)
            {
    ...
    internal override void SolveVelocityConstraints(ref SolverData data)
    {
        ...
        if (_enableLimit && _limitState != LimitState.Inactive && fixedRotation == false)
        {
            ...
            Vector3 impulse = -_mass.Solve3x3(Cdot);
            ...
            else if (_limitState == LimitState.AtLower)
            {
                ...
                if (newImpulse < 0.0)
                {
                    ...
                    Vector2 reduced = _mass.Solve2x2(rhs);
                    impulse.X = reduced.X;
                    impulse.Y = reduced.Y;
        ...
        else
        {
            ...
            Vector2 impulse = _mass.Solve2x2(-Cdot);
    ...
    internal override bool SolvePositionConstraints(ref SolverData data)
    {
        ...
        if (_enableLimit && _limitState != LimitState.Inactive && fixedRotation == false)
        {
            double angle = aB - aA - ReferenceAngle;
            double limitImpulse = 0.0;

            if (_limitState == LimitState.Equal)
            {
                // Prevent large angular corrections
                double C = MathUtils.Clamp(angle - _lowerAngle, -Settings.MaxAngularCorrection, Settings.MaxAngularCorrection);
