...
using StdbModule.Physics2D.Controllers;
...
public partial class World
{
    ...
    public World()
    {
        Island = new Island();
        Enabled = true;
        BodyList = new BodyCollection();
        JointList = new JointCollection();
        ...
        _testPointDelegateCache = new QueryReportFixtureDelegate(TestPointCallback);


        ContactManager = new ContactManager(new DynamicTreeBroadPhase());
        Gravity = new Vector2(0, -9.80665);
    }
    ...
    private void SolveTOI(ref TimeStep step, ref SolverIterations iterations)
    {
        ...
        if (_stepComplete)
        {
            ...
            for (int i = 0; i < BodyList._list.Count; i++)
            {
                ...
                BodyList._list[i]._sweep.Alpha0 = 0.0;
            }
#endif
#if USE_ACTIVE_CONTACT_SET
            foreach (var c in ContactManager.ActiveContacts)
        ...
        for (; ; )
        {
            ...
            if (minContact == null || 1.0 - 10.0 * MathUtils.Epsilon < minAlpha)
            {
...
#pragma warning restore CS0162 // Unreachable code detected
    ...
    {
        Step(dt.TotalSeconds, ref iterations);
    }
    ...
    public void Step(double dt)
    {
        SolverIterations iterations = new()
        {
            ...
            AutoClearForces = Settings.AutoClearForces
        };
    ...
    /// <param name="dt">The amount of time to simulate in seconds, this should not vary.</param>
    /// <exception cref="System.InvalidOperationException">Thrown when the world is Locked/Stepping.</exception>
    public void Step(double dt, ref SolverIterations iterations)
    {
        if (IsLocked)
            throw new InvalidOperationException("The World is locked.");
        ...
        try
        {
            ...
            // Handle TOI events.
            if (Settings.ContinuousPhysics && step.dt > 0.0)
            {
    ...
    private double RayCastCallback(ref RayCastInput rayCastInput, int proxyId)
    {
        FixtureProxy proxy = ContactManager.BroadPhase.GetProxy(proxyId);
        ...
        if (hit)
        {
            ...
            Vector2 point = (1.0 - fraction) * rayCastInput.Point1 + fraction * rayCastInput.Point2;
    ...
    {
        AABB aabb;
        Vector2 d = new(MathUtils.Epsilon, MathUtils.Epsilon);
