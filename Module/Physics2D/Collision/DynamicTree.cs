...
public class DynamicTree<TNode>
{
    ...
    public double AreaRatio
    {
        get
        {
            ...
            double rootArea = _nodes[_root].AABB.Perimeter;
    ...
    public void RayCast(BroadPhaseRayCastCallback callback, ref RayCastInput input)
    {
        ...
        Debug.Assert(r.LengthSquared > 0.0);
        ...
        while (_raycastStack.Count > 0)
        {
            ...
            double separation = Math.Abs(Vector2.Dot(new Vector2(-r.Y, r.X), p1 - c)) - Vector2.Dot(absV, h);
            if (separation > 0.0)
            {
                continue;
            }
            ...
            if (_nodes[nodeId].IsLeaf())
            {
                ...
                double value = callback(ref subInput, nodeId);
    ...
    public void RebuildBottomUp()
    {
        ...
        while (count > 1)
        {
            double minCost = MathUtils.MaxDouble;
            int iMin = -1, jMin = -1;
