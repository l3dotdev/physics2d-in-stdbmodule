...
using StdbModule.Physics2D.Common.ConvexHull;
...
public class PolygonShape : Shape
{
    ...
    /// </summary>
    /// <param name="density">The density.</param>
    public PolygonShape(double density) : base(density)
    {
    ...
    public Vertices Vertices
    {
        ...
        set
        {
            ...
            for (int i = 0; i < _vertices.Count; ++i)
            {
                ...
                Debug.Assert(edge.LengthSquared > MathUtils.Epsilon * MathUtils.Epsilon);

                //FPE optimization: Normals.Add(MathUtils.Cross(edge, 1.0));
    ...
    protected override void ComputeProperties()
    {
        ...
        for (int i = 0; i < Vertices.Count; ++i)
        {
            ...
            double D = MathUtils.Cross(ref e1, ref e2);
        ...
        Debug.Assert(area > MathUtils.Epsilon);
    ...
    public override bool TestPoint(ref Transform transform, ref Vector2 point)
    {
        ...
        for (int i = 0; i < Vertices.Count; ++i)
        {
            double dot = Vector2.Dot(Normals[i], pLocal - Vertices[i]);
            if (dot > 0.0)
            {
                return false;
            }
    ...
    public override bool RayCast(out RayCastOutput output, ref RayCastInput input, ref Transform transform, int childIndex)
    {
        ...
        for (int i = 0; i < Vertices.Count; ++i)
        {
            ...
            double numerator = Vector2.Dot(Normals[i], Vertices[i] - p1);
            double denominator = Vector2.Dot(Normals[i], d);
        ...
        Debug.Assert(0.0 <= lower && lower <= input.MaxFraction);
    ...
    public override void ComputeAABB(out AABB aabb, ref Transform transform, int childIndex)
    {
        ...
        var vert = Vertices[0];
        aabb.LowerBound.X = vert.X * transform.Rotation.Real - vert.Y * transform.Rotation.i + transform.Position.X;
        aabb.LowerBound.Y = vert.Y * transform.Rotation.Real + vert.X * transform.Rotation.i + transform.Position.Y;
        ...
        for (int i = 1; i < Vertices.Count; ++i)
        {
            ...
            vert = Vertices[i];
            double vX = vert.X * transform.Rotation.Real - vert.Y * transform.Rotation.i + transform.Position.X;
            double vY = vert.Y * transform.Rotation.Real + vert.X * transform.Rotation.i + transform.Position.Y;
    ...
    public override double ComputeSubmergedArea(ref Vector2 normal, double offset, ref Transform xf, out Vector2 sc)
    {
        ...
        double offsetL = offset - Vector2.Dot(normal, xf.Position);
        ...
        for (i = 0; i < Vertices.Count; i++)
        {
            ...
            bool isSubmerged = depths[i] < -MathUtils.Epsilon;
            if (i > 0)
            {
                if (isSubmerged)
                {
